 </main>

    <!-- Footer -->
    <footer class="footer">
        <div class="footer__container">
            <div class="footer__content">
                <div class="footer__section">
                    <h3 class="footer__title">
                        <i class="fas fa-film"></i>
                        CineVerse
                    </h3>
                    <p class="footer__text">
                        Votre portail vers l'univers du cinéma. Découvrez, explorez et partagez votre passion.
                    </p>
                </div>

                <div class="footer__section">
                    <h4 class="footer__subtitle">Navigation</h4>
                    <ul class="footer__links">
                        <li><a href="index.php" class="footer__link">Accueil</a></li>
                        <li><a href="movies.php" class="footer__link">Films</a></li>
                        <li><a href="#about" class="footer__link">À propos</a></li>
                    </ul>
                </div>

                <div class="footer__section">
                    <h4 class="footer__subtitle">Suivez-nous</h4>
                    <div class="footer__social">
                        <a href="#" class="footer__social-link" aria-label="Facebook">
                            <i class="fab fa-facebook"></i>
                        </a>
                        <a href="#" class="footer__social-link" aria-label="Twitter">
                            <i class="fab fa-twitter"></i>
                        </a>
                        <a href="#" class="footer__social-link" aria-label="Instagram">
                            <i class="fab fa-instagram"></i>
                        </a>
                    </div>
                </div>
            </div>

            <div class="footer__bottom">
                <p>&copy; 2025-2026 CineVerse. Projet ECF - Réaliser par Valentin Martin - Formation DWWM. Données fournies par TMDB.</p>
            </div>
        </div>
    </footer>

    <!-- JavaScript -->
    <script type="module" src="js/main.js"></script>
</body>
</html>